...
AI commonly mixes Base UI and Radix because both live in the same headless UI space. qyl must remove that ambiguity by
banning cross-family patterns entirely.
...
Reject code that is "close enough" if it uses the wrong primitive semantics. Similar intent does not make the API
interchangeable.
