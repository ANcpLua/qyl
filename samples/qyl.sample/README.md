...
compile time. The SDK is the seed of `Qyl.Sdk`, an MSBuild SDK for opinionated API design. This is the first alpha; it is kept small.
...
| XML | `[GenerateXml]` + `[XmlRoot]`/`[XmlAttribute]`/`[XmlElement]` on the `partial record Todo` | `Qyl.Sdk.Xml.Generator` emits `IXmlWritable.WriteXml` as plain `XmlWriter` calls | `GET /todos/{id}/xml` → `200 application/xml`, `<todo id="6">…</todo>` |
...
```
qyl.sdk/
  Sdk/Sdk.props, Sdk/Sdk.targets      entry points; today imported explicitly, later <Project Sdk="Qyl.Sdk">
  ...
  Sources/                            AddQylApi, IQylApiBuilder, [GenerateXml], IXmlWritable, XmlHttpResult, QylResults; compiled into the consumer
  Qyl.Sdk.Xml.Generator/              Roslyn incremental generator behind [GenerateXml]
