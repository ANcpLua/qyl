...
| OTLP CORS        | [x]    | QYL_OTLP_CORS_*       | Dashboard:Otlp:Cors |
...
| Metrics page            | [x]    | GenAI Analytics view  | Charts + table view |
...
| Feature         | Status | qyl Approach   | Aspire             |
|-----------------|--------|----------------|--------------------|
| Dashboard auth  | [x]    | Token + Cookie | BrowserToken/OIDC  |
| OTLP auth       | [x]    | API Key        | ApiKey/Certificate |
...
| Telemetry limits     | [x]    | TelemetryLimitsOptions      | MaxLogCount etc. |
...
**qyl advantage:** We don't need language-specific hosting APIs because we're a standalone collector. Any app that
speaks OTLP works.
...
1. [x] Dashboard auth (token + cookie auth) - TokenAuth middleware
2. [x] GenAI Analytics UI - Real data via `/api/v1/genai/*` endpoints
3. [x] Telemetry limits - TelemetryLimitsOptions + TelemetryCleanupService
4. [x] OTLP CORS - RFC 6454 compliant (no credentials with wildcard origin)
