...
Files matching `*.g.cs`, `*.g.tsp`, `*.g.sql`, `*.g.ts`, and `core/openapi/openapi.yaml` are **generated outputs**. They
are deterministic, idempotent artifacts of the generation pipeline.
...
5. **If you cannot fix the generator pipeline**, say so. Do not hand-write a `.g` file as a workaround. Ask for help
   from a plugin, skill, or the user.
6. **`nuke Generate` is idempotent.** Anyone can run it anytime. It regenerates everything. Running it is never
   destructive — it only overwrites generated files.
