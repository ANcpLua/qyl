...
- Use the Apex three-builder pattern: `IXxxChatClientBuilder` → `IXxxAgentsBuilder` → workflow code. See `eng/skill.md`
  for the contract.
- Decorate at composition root via `.AsBuilder().UseOpenTelemetry("qyl.agent").Build()` or
  `.AsBuilder().UseQylTelemetry().Build()`. Do not instantiate `ActivitySource` directly in agent code.
- `[AgentTraced]` is **removed**. Do not reintroduce it. Tracing is applied via fluent middleware at the builder level.
