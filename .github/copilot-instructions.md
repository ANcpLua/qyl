...
You are GitHub Copilot operating on the qyl repository **from the cloud** (PR review, `@copilot` mentions, Copilot
Workspace, code-completion in github.com). This file is for you only — not for local IDE agents.

Local agents (Claude Code, Codex, aider, etc.) read `AGENTS.md` (and its `CLAUDE.md` symlink). The technical rules are
duplicated below so this file remains self-contained — Copilot in the cloud cannot follow file references reliably.
...
1. Read every review comment in the thread — from Codex, CodeRabbit, Copilot AI review, GitHub Code Quality, and human
   reviewers. Apply all of them step-by-step either seperate or in a single commit (small changes <15min can be single) if no merge conflict always single``, in which case apply the stricter one.
2. After applying, all review feedback request a 2nd or 3rd or Nth agent to review to make sure nothing got lost during the process as you have to delete and update in that process without human oversight do not ignore cleaning up its utmost importancy that you can work on your own without human help. 
3. Commit message format: `fix(pr): address review feedback — <one-line summary>`. Squash if overlapping or subset fix occured; use a fresh commit per logical fix.
4. Never commit unfinished or quick fixes unless its the best option, ask two subagents for evaluating the dynamic fix you had to apply out of plan.
...
These mirror `.editorconfig`, `AGENTS.md`, and the ruleset emitted by `eng/MSBuild/Shared.Claude.*`. Follow them on
...
Code is generated by TypeSpec emitters (`nuke Generate`), the OTel Weaver (`./eng/semconv/run-weaver.sh` and
`nuke GenerateSemconv`), and three Roslyn source generators under `internal/*.generators/`. Any file ending in `.g.cs`,
`.g.ts`, `.g.sql`, or `.g.tsp` is off-limits for direct edits. To change generated output, edit the source and
regenerate:
...
- Use the qyl three-builder pattern: `IXxxChatClientBuilder` → `IXxxAgentsBuilder` → workflow code. Concrete contract
...
- Decorate at composition root. Three distinct middleware helpers in
  `internal/qyl.instrumentation/Instrumentation/GenAi/GenAiInstrumentation.cs` — do not confuse them:
    ...
    - `ChatClientBuilder` fluent form: `new ChatClientBuilder(innerClient).UseQylTelemetry(sourceName: "qyl.genai")
      ...
      `services/qyl.loom.patterns/Clients/QylLoomPatternsChatClientBuilder.cs:64`.
    - `AIAgentBuilder` fluent form: `agent.AsBuilder().UseQylAgentTelemetry().Build()` — wraps the agent. Line 141.
      See `services/qyl.loom/Agents/QylLoomAgentsBuilder.cs:77-80` for the canonical shape.
...
|-------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| **Agent — standalone**        | `llm.AsAIAgent(new ChatClientAgentOptions { Name, Description, ChatOptions = new() { Instructions } }).AsBuilder().UseQylAgentTelemetry().Build()`                                                                                           | `services/qyl.mcp/Agents/QylMcpAgentsBuilder.cs:41-49` (full options shape), `services/qyl.loom/Agents/QylLoomAgentsBuilder.cs:77-80` (Loom side) |
| **Agent — non-streaming**     | `await agent.RunAsync(userMessage, cancellationToken: ct)`                                                                                                                                                                                   | universal across `services/qyl.loom/Autofix/Workflow/Executors/*.HandleAsync` and `services/qyl.loom/TriagePipelineService.cs`                    |
| **Agent — streaming**         | `await foreach (var evt in streamingRun.WatchStreamAsync(ct)) { … }`                                                                                                                                                                         | `services/qyl.loom/Autofix/LoomAutofixRunner.cs:117,125`, `services/qyl.loom/Exploration/ExplorationOrchestrator.cs:27,37`                        |
...
| **Workflow — build**          | `new WorkflowBuilder(start).AddEdge(a, b).AddFanOutEdge(src, [t1, t2, t3]).WithOutputFrom(last).Build()`                                                                                                                                     | `services/qyl.loom/Autofix/Workflow/AutofixWorkflowFactory.cs:79-129`, `services/qyl.loom/Exploration/Workflow/ExplorationWorkflowFactory.cs:23-28` |
| **Workflow — run**            | `InProcessExecution.RunStreamingAsync(workflow, input)` + `run.WatchStreamAsync(ct)`                                                                                                                                                         | `services/qyl.loom/Autofix/LoomAutofixRunner.cs:117,125`, `services/qyl.loom/Exploration/ExplorationOrchestrator.cs:27,37`                        |
...
- `McpClient.CreateAsync(...)` (client-side MCP consumption), `HostedMcpServerTool` (OpenAI-Responses hosted MCP),
  ...
  client behind a qyl builder under `services/qyl.loom.patterns/`.
