...
// `@encodedName("application/json", ANcpLua.OtelConventions.OTel.Keys.<Domain>.<Ident>)`
// in qyl TypeSpec source files.
//
// The mapping comes from the npm package
// `@o-ancpplua/otel-conventions-api/generated/otel-keys.gen.tsp`, which is
// emitted upstream by the producer repo (ANcpLua/typespec-otel-semconv) from
// the pinned OpenTelemetry semantic-conventions release. Hand-edit of either
// the generated file or the producer template is forbidden — bump the npm
// package release instead.
//
...
//
// Usage:
//   node core/specs/scripts/apply-otel-keys.mjs            # rewrite all .tsp
//   node core/specs/scripts/apply-otel-keys.mjs --dry-run  # report only
//   node core/specs/scripts/apply-otel-keys.mjs path/to/file.tsp [...more]
//
...
// ── Build {dotted.key → "ANcpLua.OtelConventions.OTel.Keys.<Domain>.<Ident>"} from
// the package-shipped TSP.
let generated;
try {
  generated = readFileSync(GENERATED, "utf8");
} catch (err) {
  console.error(`No consts parsed from ${GENERATED}. Run \`npm install\` in core/specs first.`);
  process.exit(1);
}
