...
**Generator emit helpers come from `ANcpLua.Roslyn.Utilities` ("RU"), not hand-rolled.** The
`internal/*.generators/` projects consume RU's `.Sources` package (compiled in, `PrivateAssets="all"`, with a
`global using ANcpLua.Roslyn.Utilities`). Before writing a C# string-literal escaper, identifier sanitizer,
deterministic hash, field-name caser, or SQL-identifier quoter inside an emitter, reach for RU — it already
ships them (`EscapeCSharpString`, `SanitizeIdentifier`, `ToFnv1aHash`, `ToFieldName`, `SqlIdentifier.Quote`, …;
inspect the source for the full set, which this doc deliberately doesn't enumerate). Missing one? Add it
upstream in RU (local checkout `~/RiderProjects/ANcpLua.Roslyn.Utilities`), cut a release, and bump
`ANcpLuaRoslynUtilitiesVersion` in `Version.props` — don't re-hand-roll inside qyl.
