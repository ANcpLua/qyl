...
internal static class Program
{
    ...
    private static Command CreateGenerateOttlCommand()
    {
        ...
        {
            string targetVersion = parseResult.GetValue(targetVersionOption) ?? "1.38.0";
            FileInfo? output = parseResult.GetValue(outputOption);

            return await GenerateOttlHandler(targetVersion, output).ConfigureAwait(false);
        });

        return command;
    }

    private static async Task<int> GenerateOttlHandler(string targetVersion, FileInfo? output)
    {
        try
        {
            string config = $"""
    ...
    private static int DiffHandler(string format)
    {
        ...
        {
            case "json":
                foreach ((string old, string @new, string version) in renames)
    ...
    private static Command CreateValidateCommand()
    {
        ...
        command.SetAction(async (parseResult, _) =>
        {
            FileInfo config = parseResult.GetRequiredValue(configOption);
    ...
    private static Task<int> ValidateHandler(FileInfo config)
    {
    ...
    private static async Task<int> UpdateSchemasHandler(DirectoryInfo? outputDir)
    {
        ...
        string dir = outputDir?.FullName ?? "./schemas";
        ...
        {
            string url = $"https://opentelemetry.io/schemas/{version}";
            Console.Write($"Fetching {version}... ");

            try
            {
                string content = await http.GetStringAsync(url);
                string path = Path.Combine(dir, $"{version}.yaml");
