...
{
    private const string SqlFileHeader = """
        -- ═══════════════════════════════════════════════════════════════════════════════
        -- qyl DuckDB Schema
        -- Auto-generated by qyl emitter. Do not edit manually.
        -- Generated: {0:O}
        -- ═══════════════════════════════════════════════════════════════════════════════

        """;

    private const string CSharpFileHeader = """
        // <auto-generated/>
    ...
    public static string EmitSql(QylSchema schema)
    {
        ...
        sb.AppendFormat(SqlFileHeader, DateTime.UtcNow);
        sb.AppendLine();
    ...
    private static void EmitTable(StringBuilder sb, TableDefinition table, QylSchema schema)
    {
    ...
    private static void EmitView(StringBuilder sb, TableDefinition table)
    {
        sb.AppendLine($"-- {table.Description}");
    ...
    public static string EmitCSharpMappings(QylSchema schema, string rootNamespace)
    {
        ...
        sb.AppendFormat(CSharpFileHeader, DateTime.UtcNow);
        sb.AppendLine();
        ...
        foreach (var table in schema.Tables.Where(t => !t.IsView))
        {
            ...
            sb.AppendLine("        public static FrozenDictionary<string, string> PropertyToColumn { get; } = new Dictionary<string, string>");
            sb.AppendLine("        {");
            foreach (var prop in model.Properties?.Where(p => p.DuckDbColumn is not null) ?? [])
            {
            ...
            if (table.PrimaryKey is not null)
            {
                ...
                sb.AppendLine($"        public const string SelectByIdSql = \"SELECT * FROM {table.Name} WHERE {table.PrimaryKey} = $1\";");
                sb.AppendLine();
            }
        ...
        sb.AppendLine("    public static FrozenDictionary<string, string> GenAiAttributeToColumn { get; } = new Dictionary<string, string>");
        sb.AppendLine("    {");
