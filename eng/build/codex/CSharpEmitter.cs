...
/// <summary>
/// Emits C# code from schema definitions.
/// Generates:
...
internal static class CSharpEmitter
{
    ...
    public static string EmitPrimitive(PrimitiveDefinition primitive, string rootNamespace)
    {
        var sb = new StringBuilder();
        
        sb.AppendFormat(GeneratedFileHeader, DateTime.UtcNow);
        sb.AppendLine();
        ...
        var interfaces = string.Join(", ", primitive.Implements.Concat(new[]
        {
        ...
        // Constructor
        sb.AppendLine($"    /// <summary>Creates a new <see cref=\"{primitive.Name}\"/> from the underlying value.</summary>");
        ...
        sb.AppendLine($"    public static implicit operator {primitive.UnderlyingType}({primitive.Name} value) => value._value;");
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>Explicitly converts from the underlying type.</summary>");
        sb.AppendLine($"    public static explicit operator {primitive.Name}({primitive.UnderlyingType} value) => new(value);");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine($"    /// <inheritdoc/>");
        sb.AppendLine($"    public override bool Equals([NotNullWhen(true)] object? obj) => obj is {primitive.Name} other && Equals(other);");
        sb.AppendLine();
        sb.AppendLine($"    /// <inheritdoc/>");
        ...
        sb.AppendLine($"    public static bool operator ==({primitive.Name} left, {primitive.Name} right) => left.Equals(right);");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine($"    /// <inheritdoc/>");
        sb.AppendLine($"    public string ToString(string? format, IFormatProvider? formatProvider) => _value.ToString(format, formatProvider);");
        sb.AppendLine();
        
        // ISpanParsable
        if (primitive.Implements.Contains("ISpanParsable<" + primitive.Name + ">"))
        {
            sb.AppendLine($"    /// <summary>Parses a span of characters into a <see cref=\"{primitive.Name}\"/>.</summary>");
            sb.AppendLine($"    public static {primitive.Name} Parse(ReadOnlySpan<char> s, IFormatProvider? provider = null)");
            sb.AppendLine($"        => new({primitive.ParseMethod}(s, NumberStyles.None, provider));");
            sb.AppendLine();
            sb.AppendLine($"    /// <summary>Tries to parse a span of characters into a <see cref=\"{primitive.Name}\"/>.</summary>");
            sb.AppendLine($"    public static bool TryParse(ReadOnlySpan<char> s, IFormatProvider? provider, out {primitive.Name} result)");
            sb.AppendLine("    {");
            sb.AppendLine($"        if ({primitive.UnderlyingType}.TryParse(s, NumberStyles.None, provider, out var value))");
            sb.AppendLine("        {");
        ...
        sb.AppendLine($"    public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider, out {primitive.Name} result)");
        sb.AppendLine($"        => TryParse(s.AsSpan(), provider, out result);");
        sb.AppendLine();
        ...
        {
            sb.AppendLine();
            EmitJsonConverter(sb, primitive);
        }
        
        return sb.ToString();
    }

    private static void EmitJsonConverter(StringBuilder sb, PrimitiveDefinition primitive)
    {
        sb.AppendLine($"/// <summary>JSON converter for <see cref=\"{primitive.Name}\"/>.</summary>");
        sb.AppendLine($"public sealed class {primitive.JsonConverter} : JsonConverter<{primitive.Name}>");
        sb.AppendLine("{");
        sb.AppendLine($"    /// <inheritdoc/>");
        sb.AppendLine($"    public override {primitive.Name} Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)");
        sb.AppendLine("    {");
        sb.AppendLine($"        var str = reader.GetString();");
        sb.AppendLine($"        return str is null ? {primitive.Name}.Empty : {primitive.Name}.Parse(str);");
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine($"    /// <inheritdoc/>");
        sb.AppendLine($"    public override void Write(Utf8JsonWriter writer, {primitive.Name} value, JsonSerializerOptions options)");
        sb.AppendLine("    {");
    ...
    public static string EmitModel(ModelDefinition model, string rootNamespace)
    {
        if (model.IsEnum)
            return EmitEnum(model, rootNamespace);
    ...
    private static string EmitEnum(ModelDefinition model, string rootNamespace)
    {
        var sb = new StringBuilder();
    ...
    public static string EmitGenAiAttributes(
        ...
        string rootNamespace)
    {
        ...
        foreach (var (key, attr) in attributes.OrderBy(a => a.Key))
        {
            ...
            {
                sb.AppendLine($"    /// <para>DEPRECATED: Use <see cref=\"{KeyToConstantName(attr.ReplacedBy!)}\"/> instead.</para>");
            }
            sb.AppendLine($"    /// </summary>");
            
            if (attr.IsDeprecated)
            {
                sb.AppendLine($"    [Obsolete(\"Use {attr.ReplacedBy} instead\")]");
            }
        ...
        sb.AppendLine("    public static FrozenDictionary<string, string> Migrations { get; } = new Dictionary<string, string>");
        sb.AppendLine("    {");
    ...
    private static string KeyToConstantName(string key)
    {
