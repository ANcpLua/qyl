...
public sealed class DuckDbGenerator : IGenerator
{
    ...
    static string EmitSql(QylSchema schema)
    {
        ...
        // Emit tables
        foreach (var table in schema.Tables.Where(t => !t.IsView)) EmitTable(sb, table, schema);

        // Emit views
        foreach (var table in schema.Tables.Where(t => t.IsView)) EmitView(sb, table);
    ...
    static void EmitTable(StringBuilder sb, TableDefinition table, QylSchema schema)
    {
        ...
        // Primary key
        if (table.PrimaryKey is not null) sb.AppendLine($"    PRIMARY KEY ({table.PrimaryKey})");
    ...
    static string EmitCSharpMappings(QylSchema schema, string rootNamespace)
    {
        ...
        foreach (var table in schema.Tables.Where(t => !t.IsView))
        {
            ...
            if (promotedCols.Count > 0)
            {
                ...
                sb.AppendLine("        {");
                foreach (var prop in promotedCols) sb.AppendLine($"            {prop.Name},");
                sb.AppendLine("        }.ToFrozenSet();");
                sb.AppendLine();
            }
