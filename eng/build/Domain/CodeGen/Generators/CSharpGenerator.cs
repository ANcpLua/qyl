...
public sealed class CSharpGenerator : IGenerator
{
    ...
    public FrozenDictionary<string, string> Generate(QylSchema schema, BuildPaths paths, string rootNamespace)
    {
        ...
        {
            var content = EmitPrimitive(primitive, rootNamespace);
            outputs[$"Primitives/{primitive.Name}.g.cs"] = content;
        }

        // Generate models
        foreach (var model in schema.Models)
        {
            var content = EmitModel(model, rootNamespace);
            outputs[$"Models/{model.Name}.g.cs"] = content;
        }

        // Generate GenAiAttributes constants
        var attrsContent = EmitGenAiAttributes(schema.GenAiAttributes, rootNamespace);
        outputs["Attributes/GenAiAttributes.g.cs"] = attrsContent;
    ...
    static string EmitPrimitive(PrimitiveDefinition primitive, string rootNamespace)
    {
        ...
        if (primitive.JsonConverter is not null) sb.AppendLine($"[JsonConverter(typeof({primitive.JsonConverter}))]");
    ...
    static void EmitJsonConverter(StringBuilder sb, PrimitiveDefinition primitive)
    {
        ...
        sb.AppendLine("    /// <inheritdoc/>");
        sb.AppendLine(
            $"    public override {primitive.Name} Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("    /// <inheritdoc/>");
        sb.AppendLine(
            $"    public override void Write(Utf8JsonWriter writer, {primitive.Name} value, JsonSerializerOptions options)");
        sb.AppendLine("    {");
    ...
    static string EmitModel(ModelDefinition model, string rootNamespace)
    {
        if (model.IsEnum)
    ...
    static string EmitEnum(ModelDefinition model, string rootNamespace)
    {
        var sb = new StringBuilder();
