...
This section details how qyl handles context retrieval and analysis using a local, private approach with Ollama,
contrasting with Sentry Seer's cloud-based model.

### 25.1 Local Context Engine

Unlike Seer, which relies on GitHub app integration and cloud storage, qyl leverages its position as a local tool to
...
4. **Local Disk Access:** qyl reads the relevant source files directly from the developer's local filesystem, using the
   paths present in the stack traces or telemetry data.
5. **Prompt Construction:** A consolidated, markdown-formatted prompt is built containing the error details, the
   assembled trace context, and the raw source code.
...
1. **Model Selection:** If not explicitly configured, qyl defaults to an open-source coding model available through the
   local Ollama instance (e.g., qwen2.5-coder, llama3).
2. **Streaming Inference:** The constructed prompt is streamed to the local Ollama API (
   typically http://localhost:11434).
3. **RCA Generation:** The local model generates the Root Cause Analysis and potential code fixes based *only* on the
   provided context.
