...
interface FacadeConfig {
    ...
    enums?: Record<string, FacadeEnumDef>;
    customClasses?: Record<string, FacadeCustomClass>;
}
...
const CONFIG = {
    ...
    // C# const + UTF-8 targets dropped 2026-04-21: zero callers in src/. The facades
    // under src/qyl.contracts/Attributes/ (emitted by generateProtocolFacades) carry
    // everything we still consume. Semconv literal keys inline at call sites.
    outputs: {
...
function generateFacadeClass(data: ParsedData, facade: FacadeConfig): string {
    ...
    for (const key of facade.attributes) {
        ...
        lines.push(`    /// <summary>${key}</summary>`);
            ...
            if (enumDef.upstream) {
                ...
                if (enumDef.extensions) {
                    ...
                    for (const [prop, val] of Object.entries(enumDef.extensions)) {
                        lines.push(`        /// <summary>${val}</summary>`);
...
function main() {
    ...
    const generateAll = !tsOnly && !tspOnly && !sqlOnly && !protocolOnly;
