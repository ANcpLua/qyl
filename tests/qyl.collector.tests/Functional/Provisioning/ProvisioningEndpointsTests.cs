    ...
    : IClassFixture<ProvisioningEndpointsTests.CollectorFactory>
{
    ...
    {
        // Exercises the ORDER BY queued_at DESC contract end-to-end. The
        ...
        // generated_jobs.queued_at column is TIMESTAMP with microsecond
        ...
        returnedIds.Should().Equal(expectedOrder,
            "DuckDbStore.GetGenerationJobsByWorkspaceAsync orders by queued_at DESC; the most recently enqueued job must appear first");
    }
