...
internal sealed class QylResourceRegistry(TimeProvider time)
{
    ...
    private readonly Channel<QylResourceState> _events = Channel.CreateUnbounded<QylResourceState>(new UnboundedChannelOptions
    {
        SingleReader = true,
        SingleWriter = false,
    });
    ...
    /// <summary>Publish a state change. Stamps <see cref="QylResourceState.Timestamp"/> from the injected <see cref="TimeProvider"/>.</summary>
    public void Publish(string name, ResourceLifecycle lifecycle, int? allocatedPort = null, Uri? endpoint = null, string? lastError = null)
    {
