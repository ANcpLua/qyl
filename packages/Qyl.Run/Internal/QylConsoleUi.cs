    ...
    IHostApplicationLifetime lifetime) : BackgroundService
{
    ...
    private void Repaint(Table table)
    {
        for (var i = 0; i < resources.Count; i++)
        {
            ...
            table.UpdateCell(i, 3, new Markup(state?.Endpoint?.ToString() is { } u ? $"[link={u}]{Markup.Escape(u)}[/]" : "[grey]—[/]"));
        }
    }

    private static string StatusDot(ResourceLifecycle lifecycle) => lifecycle switch
    {
    ...
    {
        var port = state?.AllocatedPort ?? (resource.Port == QylConstants.Ports.DynamicAllocation ? (int?)null : resource.Port);
        return port?.ToString(CultureInfo.InvariantCulture) ?? "[grey]auto[/]";
    }

    private const string Footer = "[grey][[S]] Stop   [[R]] Restart   [[B]] Open browser   [[H]] Help   [[Esc]] Exit[/]";
    ...
    private async Task HandleKeysAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            if (key.Key == ConsoleKey.Escape) { lifetime.StopApplication(); return; }
            switch (char.ToUpperInvariant(key.KeyChar))
            {
                case QylConstants.Keys.Stop: lifetime.StopApplication(); return;
