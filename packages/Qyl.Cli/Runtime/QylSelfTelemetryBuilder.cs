...
/// additionally has its exporter force-disabled.
/// </summary>
internal sealed class QylSelfTelemetryBuilder
{
    private readonly QylAppBuilder _app;
    private readonly QylResourceBuilder _owner;
    private readonly QylProcessCommand _ownerCommand;
    private QylResourceBuilder? _target;

    internal QylSelfTelemetryBuilder(QylAppBuilder app, QylResourceBuilder owner, QylProcessCommand ownerCommand)
    {
        _app = app;
        _owner = owner;
    ...
    /// </summary>
    internal QylSelfTelemetryBuilder ExportToDedicatedCollector(string name, int? port = null)
    {
    ...
    internal void Apply()
    {
        if (_target is null)
        {
            throw new InvalidOperationException(
                ...
                "call ExportToDedicatedCollector(...).");
        }
