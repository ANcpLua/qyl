// <auto-generated/>
// Generated by qyl's Weaver pipeline from open-telemetry/semantic-conventions@7fe537301d17919af7d7eb65b32e9be35da2c497
// Schema: https://opentelemetry.io/schemas/1.40.0
// Licensed under Apache-2.0 (inherited from OpenTelemetry upstream)
// </auto-generated>

// Copyright (c) 2025-2026 ancplua

namespace Qyl.OpenTelemetry.SemanticConventions.Incubating.Attributes.Aspnetcore;

/// <summary>Aspnetcore Attributes (incubating).</summary>
public static class AspnetcoreAttributes
{
    /// <summary>The result of the authentication operation..</summary>
    public const string AuthenticationResult = "aspnetcore.authentication.result";

    /// <summary>Values for the <c>AuthenticationResult</c> attribute.</summary>
    public static class AuthenticationResultValues
    {
        /// <summary>Authentication failed..</summary>
        public const string Failure = "failure";
        /// <summary>No authentication information returned..</summary>
        public const string None = "none";
        /// <summary>Authentication was successful..</summary>
        public const string Success = "success";
    }

    /// <summary>The identifier that names a particular authentication handler..</summary>
    public const string AuthenticationScheme = "aspnetcore.authentication.scheme";

    /// <summary>The name of the authorization policy..</summary>
    public const string AuthorizationPolicy = "aspnetcore.authorization.policy";

    /// <summary>The result of calling the authorization service..</summary>
    public const string AuthorizationResult = "aspnetcore.authorization.result";

    /// <summary>Values for the <c>AuthorizationResult</c> attribute.</summary>
    public static class AuthorizationResultValues
    {
        /// <summary>Authorization failed..</summary>
        public const string Failure = "failure";
        /// <summary>Authorization was successful..</summary>
        public const string Success = "success";
    }

    /// <summary>The error code for a failed identity operation..</summary>
    public const string IdentityErrorCode = "aspnetcore.identity.error_code";

    /// <summary>The result from checking the password..</summary>
    public const string IdentityPasswordCheckResult = "aspnetcore.identity.password_check_result";

    /// <summary>Values for the <c>IdentityPasswordCheckResult</c> attribute.</summary>
    public static class IdentityPasswordCheckResultValues
    {
        /// <summary>Password check failed..</summary>
        public const string Failure = "failure";
        /// <summary>Password check couldn't proceed because the password was missing from the user..</summary>
        public const string PasswordMissing = "password_missing";
        /// <summary>Password check was successful..</summary>
        public const string Success = "success";
        /// <summary>Password check was successful however the password was encoded using a deprecated algorithm and should be rehashed and updated..</summary>
        public const string SuccessRehashNeeded = "success_rehash_needed";
        /// <summary>Password check couldn't proceed because the user was missing..</summary>
        public const string UserMissing = "user_missing";
    }

    /// <summary>The result of the identity operation..</summary>
    public const string IdentityResult = "aspnetcore.identity.result";

    /// <summary>Values for the <c>IdentityResult</c> attribute.</summary>
    public static class IdentityResultValues
    {
        /// <summary>Identity operation failed..</summary>
        public const string Failure = "failure";
        /// <summary>Identity operation was successful..</summary>
        public const string Success = "success";
    }

    /// <summary>Whether the sign in result was success or failure..</summary>
    public const string IdentitySignInResult = "aspnetcore.identity.sign_in.result";

    /// <summary>Values for the <c>IdentitySignInResult</c> attribute.</summary>
    public static class IdentitySignInResultValues
    {
        /// <summary>Sign in failed..</summary>
        public const string Failure = "failure";
        /// <summary>User is locked out..</summary>
        public const string LockedOut = "locked_out";
        /// <summary>User is not allowed to sign in..</summary>
        public const string NotAllowed = "not_allowed";
        /// <summary>User requires two factory authentication to sign in..</summary>
        public const string RequiresTwoFactor = "requires_two_factor";
        /// <summary>Sign in was successful..</summary>
        public const string Success = "success";
    }

    /// <summary>The authentication type..</summary>
    public const string IdentitySignInType = "aspnetcore.identity.sign_in.type";

    /// <summary>Values for the <c>IdentitySignInType</c> attribute.</summary>
    public static class IdentitySignInTypeValues
    {
        /// <summary>Sign in with a previously registered third-party login..</summary>
        public const string External = "external";
        /// <summary>Sign in with passkey..</summary>
        public const string Passkey = "passkey";
        /// <summary>Sign in with password..</summary>
        public const string Password = "password";
        /// <summary>Sign in with a two factor provider..</summary>
        public const string TwoFactor = "two_factor";
        /// <summary>Sign in with two factor authenticator app..</summary>
        public const string TwoFactorAuthenticator = "two_factor_authenticator";
        /// <summary>Sign in with two factory recovery code..</summary>
        public const string TwoFactorRecoveryCode = "two_factor_recovery_code";
    }

    /// <summary>What the token will be used for..</summary>
    public const string IdentityTokenPurpose = "aspnetcore.identity.token_purpose";

    /// <summary>Values for the <c>IdentityTokenPurpose</c> attribute.</summary>
    public static class IdentityTokenPurposeValues
    {
        /// <summary>Any token purpose that the instrumentation has no prior knowledge of..</summary>
        public const string Other = "_OTHER";
        /// <summary>The token is for changing the user email address..</summary>
        public const string ChangeEmail = "change_email";
        /// <summary>The token is for changing a user phone number..</summary>
        public const string ChangePhoneNumber = "change_phone_number";
        /// <summary>The token is for confirming user email address..</summary>
        public const string EmailConfirmation = "email_confirmation";
        /// <summary>The token is for resetting a user password..</summary>
        public const string ResetPassword = "reset_password";
        /// <summary>The token is for changing user two factor settings..</summary>
        public const string TwoFactor = "two_factor";
    }

    /// <summary>The result of token verification..</summary>
    public const string IdentityTokenVerified = "aspnetcore.identity.token_verified";

    /// <summary>Values for the <c>IdentityTokenVerified</c> attribute.</summary>
    public static class IdentityTokenVerifiedValues
    {
        /// <summary>Token verification failed..</summary>
        public const string Failure = "failure";
        /// <summary>Token verification was successful..</summary>
        public const string Success = "success";
    }

    /// <summary>The user update type..</summary>
    public const string IdentityUserUpdateType = "aspnetcore.identity.user.update_type";

    /// <summary>Values for the <c>IdentityUserUpdateType</c> attribute.</summary>
    public static class IdentityUserUpdateTypeValues
    {
        /// <summary>Any update type that the instrumentation has no prior knowledge of..</summary>
        public const string Other = "_OTHER";
        /// <summary>Identity user access failure recorded..</summary>
        public const string AccessFailed = "access_failed";
        /// <summary>Identity user claims added..</summary>
        public const string AddClaims = "add_claims";
        /// <summary>Identity user login added..</summary>
        public const string AddLogin = "add_login";
        /// <summary>Identity user password added..</summary>
        public const string AddPassword = "add_password";
        /// <summary>Identity user added to roles..</summary>
        public const string AddToRoles = "add_to_roles";
        /// <summary>Identity user email changed..</summary>
        public const string ChangeEmail = "change_email";
        /// <summary>Identity user password changed..</summary>
        public const string ChangePassword = "change_password";
        /// <summary>Identity user phone number changed..</summary>
        public const string ChangePhoneNumber = "change_phone_number";
        /// <summary>Identity user email confirmed..</summary>
        public const string ConfirmEmail = "confirm_email";
        /// <summary>Identity user new two-factor recovery codes generated..</summary>
        public const string GenerateNewTwoFactorRecoveryCodes = "generate_new_two_factor_recovery_codes";
        /// <summary>Identity user password rehashed..</summary>
        public const string PasswordRehash = "password_rehash";
        /// <summary>Identity user two-factor recovery code redeemed..</summary>
        public const string RedeemTwoFactorRecoveryCode = "redeem_two_factor_recovery_code";
        /// <summary>Identity user authentication token removed..</summary>
        public const string RemoveAuthenticationToken = "remove_authentication_token";
        /// <summary>Identity user claims removed..</summary>
        public const string RemoveClaims = "remove_claims";
        /// <summary>Identity user removed from roles..</summary>
        public const string RemoveFromRoles = "remove_from_roles";
        /// <summary>Identity user login removed..</summary>
        public const string RemoveLogin = "remove_login";
        /// <summary>Identity user passkey removed..</summary>
        public const string RemovePasskey = "remove_passkey";
        /// <summary>Identity user password removed..</summary>
        public const string RemovePassword = "remove_password";
        /// <summary>Identity user claim replaced..</summary>
        public const string ReplaceClaim = "replace_claim";
        /// <summary>Identity user access failure count reset..</summary>
        public const string ResetAccessFailedCount = "reset_access_failed_count";
        /// <summary>Identity user authenticator key reset..</summary>
        public const string ResetAuthenticatorKey = "reset_authenticator_key";
        /// <summary>Identity user password reset..</summary>
        public const string ResetPassword = "reset_password";
        /// <summary>Identity user security stamp updated..</summary>
        public const string SecurityStamp = "security_stamp";
        /// <summary>Identity user authentication token set..</summary>
        public const string SetAuthenticationToken = "set_authentication_token";
        /// <summary>Identity user email set..</summary>
        public const string SetEmail = "set_email";
        /// <summary>Identity user lockout enabled or disabled..</summary>
        public const string SetLockoutEnabled = "set_lockout_enabled";
        /// <summary>Identity user lockout end date set..</summary>
        public const string SetLockoutEndDate = "set_lockout_end_date";
        /// <summary>Identity user passkey set..</summary>
        public const string SetPasskey = "set_passkey";
        /// <summary>Identity user phone number set..</summary>
        public const string SetPhoneNumber = "set_phone_number";
        /// <summary>Identity user two-factor authentication enabled or disabled..</summary>
        public const string SetTwoFactorEnabled = "set_two_factor_enabled";
        /// <summary>Identity user updated..</summary>
        public const string Update = "update";
        /// <summary>Identity user name updated..</summary>
        public const string UserName = "user_name";
    }

    /// <summary>The full name of the identity user type..</summary>
    public const string IdentityUserType = "aspnetcore.identity.user_type";

    /// <summary>The name of the library or subsystem using the memory pool instance..</summary>
    public const string MemoryPoolOwner = "aspnetcore.memory_pool.owner";

    /// <summary>A flag indicating whether the sign in is persistent..</summary>
    public const string SignInIsPersistent = "aspnetcore.sign_in.is_persistent";

}