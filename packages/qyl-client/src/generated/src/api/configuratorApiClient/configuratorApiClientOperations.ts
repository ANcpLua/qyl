...
export function listProfiles(
  ...
  return buildPagedAsyncIterator<GenerationProfileEntity, ListProfilesPageResponse, ListProfilesPageSettings>({getElements, getPagedResponse: getPagedResponse as any});
}
...
export async function saveSelections(
  ...
  const httpRequestOptions = {
    ...
    },body: jsonGenerationSelectionSaveRequestToTransportTransform(selections),
  };
  const response = await client.pathUnchecked(path).post(httpRequestOptions);
