...
- **qyl-side tests:** **53/53 passing** on `feat/semconv-srcgen` after the all-surface stability split. Includes strict namespace-invariant coverage, stable/incubating snapshot pairs, and generator caching checks.
...
- **Phase 3 compliance audit** (`docs/weaver-audit-phase3-report.md`): **PASS** after the stability-intercept pass. The old seed-registry / `[Experimental]` / PR-A-only-matrix findings are superseded; remaining notes are intentional architecture and downstream contrib publication work.
