...
- `[AgentTraced]`
    - Target: method
    - Purpose: generates `gen_ai.agent.invoke` spans following OpenTelemetry GenAI semantic conventions for Agent SDK
      invocations.
...
- `[OTel("semantic.convention.name")]`
    ...
    - Purpose: supplies the canonical OpenTelemetry semantic-convention key for a capture attribute such as
      `[TracedTag]` or `[Tag]`.
...
- `[Counter]`, `[Histogram]`, `[Gauge]`, `[UpDownCounter]`, `[Tag]` are emitted through **one unified Meter pipeline** (
  single pipeline, shared emitter), not as separate pipelines.
