# Persistent Multi-Agent Worktree Setup (Role-based, model-agnostic)

This template creates a deterministic, idempotent worktree layout for multi-agent execution.
...
- Create 6 permanent worktrees under `.worktrees/`:
  ...
  - `agent-architect`
  - `agent-gardener`
- Branch policy per worktree: `worktree/<worktree-name>`.
- Worktree role instructions are managed only in `AGENTS.md` (Single Source of Truth).
- `CLAUDE.md` and `.codex/agent.md` are single-line pointers:
  - `Read and follow AGENTS.md in this worktree.`
- Idempotent: re-run safely; already-correct worktrees are skipped.
- Fail fast on conflicts (missing permissions/invalid paths/branch collisions).
- Do not modify repository source code; only worktree infrastructure files.

## Architecture constraints (must never be conflated)
...
### AGENTS.md
- Must include role/scope/tasks/guardrails.
- Must include:
  - `dotnet build`
  - `dotnet test`

### CLAUDE.md / .codex/agent.md
Both must be exactly:
`Read and follow AGENTS.md in this worktree.`

## Copy this script to `scripts/setup-worktrees.sh` and run
