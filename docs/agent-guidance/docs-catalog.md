# Docs catalog and quick navigation

## Primary docs

- [README.md](../README.md): quick setup, architecture summary, quick commands.
- [CLAUDE.md](../CLAUDE.md): same canonical summary plus immediate project context.
- [docs/roadmap/loom-design.md](../docs/roadmap/loom-design.md): implementation evidence and roadmap context.
- [docs/plans/2026-03-03-qyl-agui-declarative-design.md](../docs/plans/2026-03-03-qyl-agui-declarative-design.md): AG-UI design intent.
- [docs/plans/2026-03-03-qyl-agui-declarative-impl.md](../docs/plans/2026-03-03-qyl-agui-declarative-impl.md): implementation sequence.
- [docs/sdk/features/instrumentation-toolkit.md](../sdk/features/instrumentation-toolkit.md): canonical mapping of
  tracing/agent/metric and DB interception attribute pipelines.

## Guidance and rules indexes

- [.claude/rules/build-system.md](../.claude/rules/build-system.md)
- [.claude/rules/collector.md](../.claude/rules/collector.md)
- [.claude/rules/mcp-server.md](../.claude/rules/mcp-server.md)
- [.claude/rules/protocol.md](../.claude/rules/protocol.md)
- [.claude/rules/frontend.md](../.claude/rules/frontend.md)
- [.claude/rules/testing.md](../.claude/rules/testing.md)
- [.claude/rules/roslyn-generators.md](../.claude/rules/roslyn-generators.md)

## Decision and operations references

- [docs/decisions/PRINCIPLES.md](../docs/decisions/PRINCIPLES.md)
- [docs/decisions/SCOPE-TAXONOMY.md](../docs/decisions/SCOPE-TAXONOMY.md)

## Skills index

- Skills for this repo are listed in the workspace instructions and include:
  - `agent-md-refactor`
  - `nuke-build`
  - `run-tests`
  - `instrument-service`
  - `typspec-codegen`
