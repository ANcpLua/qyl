...
Current implementation status, MCP readiness, and Loom-specific known issues live in
`docs/roadmap/loom-status.md`.

Brief summary:

- qyl already implements the core Loom-like surfaces: autofix, triage, code review, webhook ingestion, agent handoff,
...
[`sdk/traced-annotations.md`](traced-annotations.md),
[`sdk/zero-cost-observability.md`](zero-cost-observability.md),
[`query/mcp-platform.md`](mcp-platform.md),
[`intelligence/ai-chat-analytics.md`](ai-chat-analytics.md).
...
| C#         | `src/qyl.instrumentation/.../SemanticConventions.g.cs`      | .NET SDK string constants              |
| C# UTF-8   | `src/qyl.instrumentation/.../SemanticConventions.Utf8.g.cs` | Collector zero-allocation OTLP parsing |
...
**Source:** [`query/mcp-platform.md`](mcp-platform.md)
...
**Source:** [`intelligence/ai-chat-analytics.md`](ai-chat-analytics.md)
...
|-----------------------------------|---------------|------------------------------------------------------------------------------------------------|
| `Agents/QylAgentBuilder.cs`       | qyl.agents    | Fluent `AIAgent` factory: wraps any provider with `InstrumentedChatClient`                     |
| `Workflows/DeclarativeEngine.cs`  | qyl.workflows | Loads `.yaml` AdaptiveDialog workflows, wires `InstrumentedChatClient`, streams `StreamUpdate` |
...
| `Microsoft.Agents.AI.Hosting`                 | 1.0.0-preview.260225.1 | qyl.agents    |
| `Microsoft.Agents.AI.Workflows.Declarative`   | 1.0.0-rc2              | qyl.workflows |
...
**Source:** [`sdk/traced-annotations.md`](traced-annotations.md)
...
**Source:** [`sdk/zero-cost-observability.md`](zero-cost-observability.md)
...
**qyl applicability:** This pattern applies to any agent loop in qyl's agents and workflows layers where an evaluator
...
### 21.1 AI Chat Analytics — Extended (from [`ai-chat-analytics.md`](ai-chat-analytics.md))
...
### 21.4 MCP Platform — Extended (from [`mcp-platform.md`](mcp-platform.md))
...
### 21.6 Traced Annotations — Stories (from [`traced-annotations.md`](traced-annotations.md))
...
`zero-cost-observability.md`](zero-cost-observability.md))
...
(per [SCOPE-TAXONOMY.md](SCOPE-TAXONOMY.md)), confidence tag, and rationale.
...
verification status. Per [SCOPE-TAXONOMY.md](SCOPE-TAXONOMY.md) Rule 4: features
...
## 25. qyl Architecture & Data Flow

This section details how qyl handles context retrieval and analysis using a local, private approach with Ollama, contrasting with Sentry Seer's cloud-based model.

### 25.1 Local Context Engine

Unlike Seer, which relies on GitHub app integration and cloud storage, qyl leverages its position as a local tool to directly access context.

**Data Retrieval Pipeline:**

1.  **Event Ingestion:** qyl collects telemetry (errors, spans, logs) via its OTLP receiver and stores it in DuckDB.
2.  **Trigger:** An analysis request is initiated (e.g., via the MCP tool analyze_trace_with_qyl).
3.  **Trace Assembly:** qyl queries DuckDB to assemble the full trace context associated with the trigger.
4.  **Local Disk Access:** qyl reads the relevant source files directly from the developer's local filesystem, using the paths present in the stack traces or telemetry data.
5.  **Prompt Construction:** A consolidated, markdown-formatted prompt is built containing the error details, the assembled trace context, and the raw source code.

### 25.2 Local Inference (Ollama)

qyl prioritizes privacy and cost-efficiency by executing inference locally using Ollama.

**Execution Flow:**

1.  **Model Selection:** If not explicitly configured, qyl defaults to an open-source coding model available through the local Ollama instance (e.g., qwen2.5-coder, llama3).
2.  **Streaming Inference:** The constructed prompt is streamed to the local Ollama API (typically http://localhost:11434).
3.  **RCA Generation:** The local model generates the Root Cause Analysis and potential code fixes based *only* on the provided context.

### 25.3 Architectural Differences: qyl vs. Sentry Seer

| Feature | Sentry Seer | qyl |
| :--- | :--- | :--- |
| **Inference Location** | Sentry Cloud (Proprietary Models) | Local (Ollama) |
| **Code Access** | GitHub App Integration | Direct Local File System Access |
| **Data Privacy** | Subject to Sentry Data Policy | 100% Local, Data Never Leaves Machine |
| **Cost** | 40/contributor/month | Free (Compute cost only) |
| **Similar Issue Grouping** | Jina Embeddings v2 + pgvector | Heuristic Fingerprinting (DuckDB) |
