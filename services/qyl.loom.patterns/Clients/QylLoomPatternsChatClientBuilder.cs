    ...
    : IQylLoomPatternsChatClientBuilder, IDisposable
{
    ...
    {
        ["rca"]      = "Connection pool exhausted after deploy at 14:02 — stripe-webhook handler leaks SqlConnection on timeout path.",
        ["solution"] = "Wrap the timeout branch in a `using` block; add a connection-pool-exhaustion Polly retry for 503s.",
        ["verdict"]  = "approved",
    };
    ...
    public IChatClient BuildChatClient(string stage)
    {
        ...
        var fake = new FakeChatClient
        {
            Metadata = new ChatClientMetadata(
                providerName:    "fake",
                providerUri:     null,
                defaultModelId:  $"loom-patterns-{stage}"),
        };
