...
internal sealed partial class WorkflowCheckpointFileSystem : IDisposable
{
    ...
    public bool DeleteEmptyDirectory(string path)
    {
        try
        {
            ...
            if (error is ErrorNoEntry)
                return true;
            if (error is ErrorDirectoryNotEmpty)
                return false;
            ThrowPathError(error, path);
            return false;
        }
    ...
    public IEnumerable<WorkflowCheckpointFileSystemEntry> EnumerateTree(string path)
    {
        ...
        try
        {
            directory = OpenRelativeDirectory(path, create: false);
        }
        catch (FileNotFoundException)
        {
            yield break;
        ...
        using (directory)
        {
            foreach (var entry in EnumerateDirectory(directory, Path.GetFullPath(path)))
                yield return entry;
        }
    }
    ...
    private IEnumerable<WorkflowCheckpointFileSystemEntry> EnumerateDirectory(
        SafeFileHandle directory,
        string path)
    {
        foreach (var name in ReadDirectoryNames(directory))
        {
            ...
            var childDirectory = TryOpenDirectoryAt(directory, name);
            if (childDirectory is not null)
            {
                using (childDirectory)
                {
                    foreach (var descendant in EnumerateDirectory(
                                 childDirectory,
                                 childPath))
                    {
                ...
                yield return new WorkflowCheckpointFileSystemEntry(
                    childPath,
                    0,
                    IsDirectory: true);
                continue;
            ...
            var file = TryOpenFileAt(directory, name);
            if (file is null)
                continue;
            using (file)
            {
    ...
    private static void ThrowPathError(int error, string path)
    {
        ...
        if (error is ErrorLoop)
        {
            throw new InvalidDataException(
                "Workflow checkpoint path contains a symbolic link.");
        }
        if (error is ErrorIsDirectory)
        {
            throw new InvalidDataException(
