...
// The awaited projection key stopped being current — retired by repair, or its
// run generation rotated. The run itself may still exist: callers re-observe
// the run row and either retry with the fresh row or report absence.
internal sealed class WorkflowProjectionRetiredException(string message)
    : Exception(message);
...
internal sealed class WorkflowProjectionRuntime : IAsyncDisposable
{
        ...
        CancellationToken ct)
    {
        ...
        var armed = waiter!;
        armed.ArmCancellation(
            ct,
            () => CancelWaiter(key, armed, ct));
        return armed.Task;
    }
        ...
        async Task DrainReadyDemandsAsync()
        {
            await foreach (var ready in _ready.Reader.ReadAllAsync(_shutdown.Token).ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    if (step.Gone)
                    {
                        CompleteDemandAsRetired(key, demand);
                        continue;
                    }
    ...
    private void CompleteDemandAsRetired(WorkflowProjectionKey key, Demand expected)
    {
        List<Waiter> waiters;
        Demand demand;
        lock (_sync)
    ...
    private static void CompleteRetired(
        IEnumerable<Waiter> waiters,
        WorkflowProjectionKey key)
    {
        var error = Retired(key);
        foreach (var waiter in waiters)
            waiter.Fail(error);
    }

    private static WorkflowProjectionRetiredException Retired(WorkflowProjectionKey key) =>
        new($"Workflow run '{key.RunId}' generation '{key.RunGeneration}' was retired.");
