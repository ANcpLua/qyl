...
internal sealed partial class DuckDbStore
{
        ...
        CancellationToken ct = default)
    {
        ...
        while (true)
        {
            observed = await GetWorkflowRunAsync(projectId, runId, ct).ConfigureAwait(false);
            if (observed is null)
                return null;
            try
            {
                checkpoint = await AwaitWorkflowProjectionAsync(observed, ct).ConfigureAwait(false);
                break;
            }
            catch (WorkflowProjectionRetiredException)
            {
                // Retirement rotated the key under us; the next observation sees
                // the current row (or the run's true absence).
            }
        }
