...
internal sealed class WorkflowCheckpointStore : IDisposable
{
    ...
    private Dictionary<string, WorkflowCheckpointPhysicalEntry> _physicalEntries =
        new(StringComparer.Ordinal);
    private static readonly TimeSpan s_temporarySidecarGrace = TimeSpan.FromMinutes(15);
        ...
        CancellationToken ct)
    {
        if (_root is null)
        {
            ...
            try
            {
                await using var buffer = new WorkflowCheckpointMemoryStream(_limits.MaxCheckpointBytes);
        ...
        else
        {
                       ...
                       limit))
            {
                ...
                candidates.Add(new WorkflowCheckpointSweepCandidate(
                    ...
                    length,
                    RetainWithoutLookup:
                        IsProtected(file) || IsWritableTemporarySidecar(file),
                    IsDirectory: false));
            }
    ...
    private async Task<string> SerializeAsync(
        ...
        CancellationToken ct)
    {
        await using var capped = new WorkflowCheckpointWriteStream(
            destination,
    ...
    // A temporary sidecar names no checkpoint identity, so the sweep would treat
    // it as an orphan. Only this process's in-flight writes are covered by
    // _pending: a sidecar written by another writer is invisible here, and
    // deleting it would destroy a checkpoint mid-write. Sidecars are therefore
    // retained until they are old enough that no writer can still own them;
    // crash residue outlives the grace period and is swept normally.
    private bool IsWritableTemporarySidecar(string path)
    {
        ...
        var lastWrite = _files!.LastWriteTimeUtc(path);
        return lastWrite != DateTime.MinValue &&
               TimeProvider.System.GetUtcNow() - lastWrite < s_temporarySidecarGrace;
    }
