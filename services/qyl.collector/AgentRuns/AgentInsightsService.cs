...
public sealed class AgentInsightsService(DuckDbStore store)
{
    ...
    public Task<ModelTimeseriesResult> GetLlmCallsAsync(
        long fromMs, long toMs, string? bucket = null, CancellationToken ct = default)
    {
        var interval = BucketInterval(bucket, fromMs, toMs);
        ...
        return store.ExecuteReadAsync(con =>
        {
            ...
            return ReadModelTimeseries(cmd);
        }, ct);
    }


    public Task<ModelTimeseriesResult> GetTokensAsync(
        long fromMs, long toMs, string? bucket = null, CancellationToken ct = default)
    {
        var interval = BucketInterval(bucket, fromMs, toMs);
        ...
        long fromMs, long toMs, string? bucket = null, int topN = 8, CancellationToken ct = default)
    {
        ...
        return await store.ExecuteReadAsync(con =>
        {
            using var topCmd = con.CreateCommand();
            topCmd.CommandText = """
            ...
            using (var tr = topCmd.ExecuteReader())
            {
                while (tr.Read())
                {
    ...
    public Task<ModelsResult> GetModelsAsync(
        long fromMs, long toMs, string? bucket = null, CancellationToken ct = default)
    {
        var interval = BucketInterval(bucket, fromMs, toMs);
        ...
        return store.ExecuteReadAsync(con =>
        {
            ...
            using var tsCmd = con.CreateCommand();
            ...
            var timeseries = ReadModelTimeseries(tsCmd);
        ...
        long fromMs, long toMs, string? bucket = null, CancellationToken ct = default)
    {
        ...
        return await store.ExecuteReadAsync(con =>
        {
            ...
            using (var reader = tsCmd.ExecuteReader())
            {
                while (reader.Read())
                {
                    var time = reader.GetDateTime(0).ToString("o");
    ...
    {
        return await store.ExecuteReadAsync<IReadOnlyList<TraceSpan>>(con =>
        {
            using var cmd = con.CreateCommand();
    ...
    private static ModelTimeseriesResult ReadModelTimeseries(DuckDBCommand cmd)
    {
        var bucketMap = new SortedDictionary<string, Dictionary<string, long>>();
        var totals = new Dictionary<string, long>();
