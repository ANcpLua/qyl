...
export const telemetryKeys = {
  ...
  resources: () => [...telemetryKeys.all, 'resources'] as const,
...
export function useSessions() {
  return useQuery({
    ...
    queryFn: () => fetchJson<{ sessions: Session[] }>('/api/v1/sessions'),
...
export function useSession(sessionId: string) {
  return useQuery({
    ...
    queryFn: () => fetchJson<{ session: Session }>(`/api/v1/sessions/${sessionId}`),
...
export function useSessionSpans(sessionId: string) {
  return useQuery({
    ...
    queryFn: () => fetchJson<{ spans: Span[] }>(`/api/v1/sessions/${sessionId}/spans`),
...
export function useTrace(traceId: string) {
  return useQuery({
    ...
    queryFn: () => fetchJson<{ spans: Span[] }>(`/api/v1/traces/${traceId}`),
    select: (data) => data.spans,
    enabled: !!traceId,
  });
}

// Resources (derived from sessions/spans)
export function useResources() {
  return useQuery({
    queryKey: telemetryKeys.resources(),
    queryFn: () => fetchJson<{ resources: Resource[] }>('/api/v1/resources'),
