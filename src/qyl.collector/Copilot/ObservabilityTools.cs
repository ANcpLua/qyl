...
internal static class ObservabilityTools
{
    ...
    [
        AIFunctionFactory.Create(
            [Description(
                "Search spans by service name, status code, and time range. Returns matching spans with timing and GenAI metadata.")]
            ...
            "search_spans",
            "Search spans by service name, status code, and time range"),
        ...
        AIFunctionFactory.Create(
            [Description(
                "Get GenAI usage statistics including request counts, token usage, and costs over a time range. Optionally filter by session.")]
            ...
            "get_genai_stats",
            "Get GenAI usage statistics (requests, tokens, costs)"),
        ...
        AIFunctionFactory.Create(
            ...
            "search_logs",
            "Search logs by severity and body text"),

        AIFunctionFactory.Create(
            [Description(
                "Get storage statistics including span count, log count, session count, and time range of stored data.")]
            ...
            "get_storage_stats",
            "Get storage statistics (counts, size)"),
        ...
        AIFunctionFactory.Create(
            ...
            "list_sessions",
            "List spans for a session by session ID"),

        AIFunctionFactory.Create(
            [Description(
                "Get pre-computed system context: topology, performance profile, and alerts. Returns materialized insights that are refreshed every 5 minutes with zero query cost.")]
            async (CancellationToken ct) =>
            {
            ...
            "get_system_context",
            "Get pre-computed system context (topology, performance, alerts)")
    ];
}
