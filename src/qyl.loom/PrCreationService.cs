    ...
    ILogger<PrCreationService> logger)
{
    ...
    {
        var run = await store.GetFixRunAsync(runId, ct).ConfigureAwait(false);
        if (run is null)
        ...
        CancellationToken ct)
    {
        ...
        var patched = TryApplyHunks(file, currentContent);
        if (patched is null)
        {
        ...
        var contentBase64 = Convert.ToBase64String(Encoding.UTF8.GetBytes(patched));

        var commitMessage = $"fix: {file.Path} — qyl autofix run {runId[..Math.Min(8, runId.Length)]}";
        var ok = await github.CreateOrUpdateFileAsync(
        ...
        IReadOnlyList<string> replacementLines)
    {
        ...
        var origLen = originalLines.Count;

        for (var i = 0; i <= contentLines.Length - origLen; i++)
        {
            var match = true;
        ...
        List<string> committed, List<string> errors)
    {
        ...
        sb.AppendLine(patch.PrBody ?? $"Automated fix generated by qyl Loom for issue `{run.IssueId}`.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("---");
        sb.AppendLine("*Generated by [qyl](https://qyl.io) Loom autofix pipeline.*");
        return sb.ToString();
    }
...
// PatchDocument, PatchFile, PatchHunk, PrCreationJsonContext live in qyl.collector/Autofix/PrCreationService.cs
