...
public sealed class InMemoryTelemetryStore<T> : ITelemetryStore<T> where T : class
{
    ...
    public void Add(T item)
    {
        ...
        while (_count > _maxItems && _items.TryDequeue(out _))
        {
    ...
    public IReadOnlyList<T> Query(TelemetryQuery query)
    {
        ...
        {
            items = items.Where(i => GetServiceName(i) == query.ServiceName);
        }

        if (query.From.HasValue)
        {
            items = items.Where(i => GetTimestamp(i) >= query.From.Value);
        }

        if (query.To.HasValue)
        {
            items = items.Where(i => GetTimestamp(i) <= query.To.Value);
        }
    ...
    private static string? GetServiceName(T item) => item switch
    {
    ...
    private static DateTimeOffset GetTimestamp(T item) => item switch
    {
