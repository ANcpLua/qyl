...
public sealed class BuildTools(HttpClient client)
{
        ...
        [Description("Build failure id")] string id)
    {
        ...
        {
            if (await client.GetFromJsonAsync<BuildFailureDto>($"/api/v1/build-failures/{Uri.EscapeDataString(id)}",
                    BuildJsonContext.Default.BuildFailureDto).ConfigureAwait(false) is not { } item)
