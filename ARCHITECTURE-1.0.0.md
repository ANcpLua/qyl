...
> **Known gap (measured 2026-07-26).** Today the collector composes self-telemetry through an internal fork — `qyl/internal/qyl.instrumentation` (`AddQylServiceDefaults`) — with measured drift against the published stack: a hardcoded activity-source list beside the generated inventory, a copy-pasted second discovery implementation, and agent-framework packages pulled into the collector's dependency closure. The ledger's fold row retires the fork; G7's edge assertion keeps it retired. Until the fold lands, the fork is a delete target, not a maintenance surface.
>
> **Measured annotation 2026-07-28 (gap closed; this note is stale, rewrite is Alex's).** Every drift symptom above measures false, and the ledger has carried this row as *Done 2026-07-26* / re-verified at closure level 2026-07-27 — so the decree and the audit disagree, with the audit correct. Today: `ActivitySources` declares one constant and no inventory; no second discovery implementation exists (the composition sets `EnableCollectorDiscovery` on the published options); `dotnet list package --include-transitive` on the collector returns 55 libraries and zero agent-framework packages. The named entry point `AddQylServiceDefaults` no longer exists either — it was a forwarder to `UseQyl`, deleted 2026-07-28. What survives the correction is the prohibition, not the diagnosis: re-deriving OTel wiring, an ActivitySource inventory, or collector discovery inside the collector is still how the fork started, and G6/G7 are unchanged by this annotation.
